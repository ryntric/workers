...
public final class DisruptorWorkerService extends WorkerService {
    ...
    private void init() {
        ...
        WorkerNodeFactory.createAndConsume(workers, config.getReplicaCount(),
                node -> nodes.put(WorkerUtil.getKeyHash(node.getName(), config.getHashFunction()), node));
    }
    ...
    @Override
    long getWorkerNodeId(String key) {
        SortedMap<Long, WorkerNode> tailed = nodes.tailMap(WorkerUtil.getKeyHash(key, config.getHashFunction()));
        return tailed.isEmpty() ? nodes.firstKey() : tailed.firstKey();
    }

    @Override
    WorkerNode getWorkerNode(String key) {
    ...
    @Override
    public CompletableFuture<Void> execute(String key, String taskName, Runnable runnable) {
        checkLiveness();
        WorkerNode node = getWorkerNode(key);
        CompletableFuture<Void> future = new CompletableFuture<>();
        node.execute(new VoidTask(key, future, runnable, taskName));
        return future;
    }

    @Override
    public <T> CompletableFuture<T> execute(String key, String taskName, Callable<T> callable) {
        checkLiveness();
        WorkerNode node = getWorkerNode(key);
        CompletableFuture<T> future = new CompletableFuture<>();
        node.execute(new CallableTask<>(key, future, callable, taskName));
        return future;
    }
